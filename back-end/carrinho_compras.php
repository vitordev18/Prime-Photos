...
if (isset($_POST['finalizar'])) {
    $stmt = $pdo->prepare("INSERT INTO compra (fk_usuario, data, acrescimo_total, sessao, status) VALUES (?, NOW(), ?, ?, ?)");
    $stmt->execute([$_SESSION['usuario_id'], 0, session_id(), 'reservado']);

    $fk_compra = $pdo->lastInsertId();
    $stmt_produto = $pdo->prepare("INSERT INTO compra_produto (fk_produto, fk_compra, quantidade, valor_unitario) VALUES (?, ?, ?, ?)");
    ...
    foreach ($_SESSION['carrinho'] as $item) {
        ...
        $stmt_produto->execute([$fk_produto, $fk_compra, $quantidade, $valor_unitario]);
    }
            ...
            <?php else: ?>
                ...
                <div class="carrinho-item">
                    <div class="item-info">
                        ...
                        <div class="item-preco">R$ <?php echo number_format($item['valor_unitario'], 2, ',', '.'); ?></div>
                    </div>
                    
                    <div class="estoque-controls estoque<?php echo $item['id'] ?>-controls">
                        <button class="estoque-btn estoque<?php echo $item['id'] ?>-btn" onclick="window.location.href='carrinho_compras.php?action=decrease&id_produto=<?php echo $item['id']; ?>'">-</button>
                        <span class="estoque-number estoque<?php echo $item['id'] ?>-number"><?php echo $item['quantidade']; ?></span>
                        <button class="estoque-btn estoque<?php echo $item['id'] ?>-btn" onclick="window.location.href='carrinho_compras.php?action=increase&id_produto=<?php echo $item['id']; ?>'">+</button>
                    </div>
                    ...
                    <button class="remove-btn" onclick="window.location.href='carrinho_compras.php?action=remove&id_produto=<?php echo $item['id']; ?>'">Remover</button>
                </div>
                <?php endforeach; ?>
                
                <div class="carrinho-total">
                    <h2>Total: <span class="total-valor">R$ <?php echo number_format($total_valor, 2, ',', '.'); ?></span></h2>
